...
export default function LazyList({ items, focused = [0] }: LazyListProps) {
	...
	const onScroll = (e: any) => {
		...
		if (scrollPercentage > 0.8 && forwardDelta < maxForwardDelta) {
			setForwardDelta(forwardDelta + 1);
		}

		if (scrollPercentage < 0.2 && reverseDelta < maxReverseDelta) {
			setReverseDelta(reverseDelta + 1);
		}
	};

	return (
		<List onScroll={onScroll}>{items.slice(startInclusive, endExclusive)}</List>
	);
}
